...
import { getMovieCredits, getMovieDetails, getMovieSimilar, getMovieVideos } from '../api/tmdbApi';
...
const MovieDetails = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setError(err?.response?.data?.message || 'Unable to load movie details.');
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    const resolveAndFetch = async () => {
      ...
      try {
        ...
        // Silently continue if backend movie sync is unavailable
      }
    };

    resolveAndFetch();
  }, [isAuthenticated, movie, tmdbId, user?.id]);
  ...
  const handleReviewSubmit = async (e) => {
    ...
    if (!backendMovieId) {
      setReviewError('Unable to link movie for review.');
      return;
    }
  ...
  if (loading) return <div className="mx-auto max-w-7xl px-4 py-10"><Spinner label="Loading movie details..." /></div>;
  ...
  const directors = crew.filter((p) => p.job === 'Director' || p.department === 'Directing').slice(0, 3);
  const writers = crew.filter((p) => p.job === 'Screenplay' || p.job === 'Writer').slice(0, 3);
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <section className="relative overflow-hidden rounded-3xl border border-white/10 bg-slate-900 shadow-2xl">
        {movie.backdrop_path && (
          <img src={`${BACKDROP_BASE}${movie.backdrop_path}`} alt={movie.title} className="absolute inset-0 h-full w-full object-cover opacity-25" />
        )}
        <div className="relative grid gap-8 p-6 md:grid-cols-[280px_1fr] md:p-10">
          <img
            src={movie.poster_path ? `${IMAGE_BASE}${movie.poster_path}` : '/No-Poster.png'}
            alt={movie.title}
          ...
          <div className="space-y-5">
            <div className="flex flex-wrap items-center gap-3">
              <h1 className="text-4xl font-black text-white text-left">{movie.title}</h1>
              <span className="rounded-full border border-violet-400/40 bg-violet-500/10 px-3 py-1 text-xs text-violet-200 font-semibold">Movie</span>
            </div>

            <div className="flex flex-wrap gap-5 text-sm text-slate-300">
              <span className="flex items-center gap-1 font-semibold text-amber-300">⭐ {Number(movie.vote_average || 0).toFixed(1)}</span>
            ...
            <p className="max-w-3xl text-slate-200 leading-relaxed text-base">{movie.overview || 'No overview available.'}</p>
      ...
      {(directors.length > 0 || writers.length > 0) && (
        <section className="mt-12">
          <h2 className="mb-4 text-2xl font-bold text-white">🎥 Key Crew</h2>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {directors.map((person, idx) => (
              <div key={`dir-${person.id}-${idx}`} className="rounded-xl border border-white/10 bg-slate-900/80 p-4">
                <span className="text-xs uppercase tracking-wider text-violet-400 font-semibold">Director</span>
                <h3 className="mt-1 font-bold text-white">{person.name}</h3>
              </div>
            ))}
            {writers.map((person, idx) => (
              <div key={`writ-${person.id}-${idx}`} className="rounded-xl border border-white/10 bg-slate-900/80 p-4">
                <span className="text-xs uppercase tracking-wider text-violet-400 font-semibold">{person.job || 'Writer'}</span>
                <h3 className="mt-1 font-bold text-white">{person.name}</h3>
              </div>
            ))}
          </div>
      ...
      <section className="mt-14 border-t border-white/10 pt-10">
        <div className="mb-6 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-400">Share your thoughts with other moviegoers</p>
          </div>
        </div>
      ...
      {similar.length > 0 && (
        <section className="mt-14">
          <h2 className="mb-4 text-2xl font-bold text-white">✨ Similar Movies You May Like</h2>
          <div className="grid gap-4 grid-cols-2 sm:grid-cols-3 lg:grid-cols-4">
            {similar.slice(0, 8).map((item) => (
              ...
              >
                ...
                />
                <div className="p-3">
                  <h3 className="line-clamp-1 text-white font-semibold text-sm">{item.title}</h3>
                  <div className="flex items-center justify-between text-xs text-slate-400 mt-1">
                    <span>{item.release_date || 'Unknown date'}</span>
